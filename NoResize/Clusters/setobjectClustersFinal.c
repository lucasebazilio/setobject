...
set_add_entry(PySetObject *so, PyObject *key, Py_hash_t hash)
{
    ...
    while (1) {
        ...
        /* Initialization of dictionary of clusters
        if (!so->clusters) {
            so->clusters = calloc(so->mask + 1, sizeof(Py_ssize_t));
            if (!so->clusters) {
                PyErr_NoMemory();
                return -1;
            }
    ...
    if (i != so->mask && i != 0) {  // We are in a general case
        ...
        Py_ssize_t *count = &so->clusters[p+1]; // + 1 since we include the empty slot
        (*count)++;
        count = &so->clusters[p];                   // Decrement old value
        (*count)--;
        count = &so->clusters[1];                   // Decrement cluster empty slot
        (*count)--;
    }
        ...
        Py_ssize_t *count = &so->clusters[q+1]; // + 1 since we include the empty slot
        (*count)++;
        count = &so->clusters[q];                   // Decrement old value
        (*count)--;
        count = &so->clusters[1];                   // Decrement cluster empty slot
        (*count)--;
    ...
    // Element where left-cluster has size p and right-cluster has size q
        ...
        for (Py_ssize_t j = i; j >= 1 && (so->table[j].hash != 0 && so->table[j].key != NULL); j--) {
            ++p;
            if (j == i) --p;
        }
        ...
        for (Py_ssize_t j = i; j < so->mask && (so->table[j].hash != 0 && so->table[j].key != NULL); j++) {
            ++q;
            if (j == i) --q;
        }
    ...
    else if (i == so->mask) {   // We are at the last slot of the hash table

        // Element between empty slots
        if (so->table[i-1].hash == 0 && so->table[i-1].key == NULL
            && so->table[0].hash == 0 && so->table[0].key==NULL) {
            Py_ssize_t *count = &so->clusters[2];
            (*count)++;
            count = &so->clusters[1];
            (*count) = (*count) - 2;
    ...
    // Element where left-cluster has size p and right-cluster has size q
    ...
    else if (i == 0) {      // We are at the first slot of the hash table

        // Element between empty slots
        if (so->table[so->mask].hash == 0 && so->table[so->mask].key == NULL
            && so->table[i+1].hash == 0 && so->table[i+1].key==NULL) {
            Py_ssize_t *count = &so->clusters[2];
            (*count)++;
            count = &so->clusters[1];
        ...
        // Element where left-cluster has size >=1
        else if (so->table[so->mask].hash != 0 && so->table[so->mask].key != NULL
        && so->table[i+1].hash == 0 && so->table[i+1].key==NULL) {
        int p = 1;
        for (j = i; j > 0 && (so->table[j].hash != 0 && so->table[j].key != NULL); j--) {
            ++p;
        }
    ...
    /*
    if (PyLong_CheckExact(key) && so->fill == 25) {
    ...
    // print the number of clusters of each size
    for (Py_ssize_t i = 0; i <= so->mask; i++) {
        Py_ssize_t count = so->clusters[i];
        if (count > 0) {
            printf("Size %zd: %zd\n", i , count);
        }
    }
    ...
    if (PyLong_CheckExact(key) && so->fill == 40000) {
    //printf("Inserting key: ");
    //PyObject_Print(key, stdout, 0);
    //printf(" with hash: %llu ", hash);
    //printf("%d ", so->num_linear_probes);
    ...
    /*
    Py_ssize_t max = 1;
    for (Py_ssize_t j = so->mask; j >= 0; j--) {
        if (so->clusters[j] > 0) max = j; break; }
...
make_new_set(PyTypeObject *type, PyObject *iterable)
{
    ...
    (*count) = 65536;    // Initially we have M clusters of size 1
